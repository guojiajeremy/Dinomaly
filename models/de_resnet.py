...
class ResNet(nn.Module):
    ...
    def __init__(
        ...
        self.layer3 = self._make_layer(block, 64, layers[0], stride=2,
        ...
        if output_conv > 0:
            ...
            self.out3 = nn.ModuleList()
            for i in range(output_conv):
                self.out1.append(nn.Conv2d(64 * block.expansion, 64 * block.expansion, 1))
                self.out2.append(nn.Conv2d(128 * block.expansion, 128 * block.expansion, 1))
                self.out3.append(nn.Conv2d(256 * block.expansion, 256 * block.expansion, 1))
            if uncertainty_conv:
                self.unc1 = nn.ModuleList()
                self.unc2 = nn.ModuleList()
                self.unc3 = nn.ModuleList()
                for i in range(output_conv):
                    self.unc1.append(nn.Conv2d(64 * block.expansion, 1, 1))
                    self.unc2.append(nn.Conv2d(128 * block.expansion, 1, 1))
                    self.unc3.append(nn.Conv2d(256 * block.expansion, 1, 1))
    ...
    def _forward_impl(self, x: Tensor) -> Tensor:
        ...
        if self.output_conv > 0:
            out = []
            for i in range(len(self.out1)):
                out.append(self.out1[i](f1))
                out.append(self.out2[i](f2))
                out.append(self.out3[i](f3))
            return out
...
def de_wide_resnet222(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    r"""Wide ResNet-50-2 model from
    `"Wide Residual Networks" <https://arxiv.org/pdf/1605.07146.pdf>`_.
    The model is the same as ResNet except for the bottleneck number of channels
    which is twice larger in every block. The number of channels in outer 1x1
...
def de_wide_resnet111(pretrained: bool = False, progress: bool = True, **kwargs: Any) -> ResNet:
    r"""Wide ResNet-50-2 model from
    `"Wide Residual Networks" <https://arxiv.org/pdf/1605.07146.pdf>`_.
    The model is the same as ResNet except for the bottleneck number of channels
    which is twice larger in every block. The number of channels in outer 1x1
    ...
    """
    kwargs['width_per_group'] = 64 * 2
    return _resnet('wide_resnet50_2', Bottleneck, [1, 1, 1, 1],
                   pretrained, progress, **kwargs)
