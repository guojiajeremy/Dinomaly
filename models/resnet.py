...
# from .condconv2 import CondConv2d
...
def cdconv3x3(in_planes: int, out_planes: int, stride: int = 1, groups: int = 1, dilation: int = 1):
    """3x3 convolution with padding"""
...
class BottleneckCond(nn.Module):
    ...
    def __init__(
        ...
        self.conv1 = conv1x1(inplanes, width)
        self.bn1 = norm_layer(width)
        self.conv2 = cdconv3x3(width, width, stride, groups, dilation)
        self.bn2 = norm_layer(width)
        self.conv3 = conv1x1(width, planes * self.expansion)
        self.bn3 = norm_layer(planes * self.expansion)
        self.relu = nn.ReLU(inplace=True)
...
class BN_layer(nn.Module):
    def __init__(self,
        ...
        # if additional_proj:
        #     self.proj_1 = ProjLayer(64 * block.expansion, 64 * block.expansion)
        #     self.proj_2 = ProjLayer(128 * block.expansion, 128 * block.expansion)
        #     self.proj_3 = ProjLayer(256 * block.expansion, 256 * block.expansion)
    ...
    def forward(self, x):
        ...
        #     x = [self.proj_1(x[0]), self.proj_2(x[1]), self.proj_3(x[2])]
